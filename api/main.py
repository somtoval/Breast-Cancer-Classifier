...
def read_file_image(data) -> np.ndarray:
    image = Image.open(BytesIO(data))# Pillow is used ot read images and we would use it to read the bytes as a pillow image and then convert it to numpy array
...
async def predict(
    ...
    print(f'>>image batch type : {type(img_batch)}')
